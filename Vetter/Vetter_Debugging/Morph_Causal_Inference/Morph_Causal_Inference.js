...
let nodes_ori = JSON.parse(fs.readFileSync('src/changba.com_ori.har')).log.entries;
let parsed_ori = parseNodesFromHar(nodes_ori);
let nodes_comp = JSON.parse(fs.readFileSync('src/changba.com_comp.har')).log.entries;
let parsed_comp = parseNodesFromHar(nodes_comp);

let size_map_ori = getFileInfoFromFolder('src/changba_ori_files');
let map_ori = mappingHarToFile(parsed_ori, size_map_ori);
// fs.writeFileSync('ori.json', JSON.stringify(parsed_ori))
// console.log(map_ori)
// console.log(parsed_ori)
let size_map_comp = getFileInfoFromFolder('src/changba_comp_files');
