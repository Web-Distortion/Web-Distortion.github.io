...
func (p *Proxy) proxyResponse(w *ResponseWriter, r *ResponseReader, headers http.Header, source_type string) error {
	w.takeHeaders(r)
	transcoder, found := p.transcoders[source_type]
	if !found {
		return w.ReadFrom(r)
	}
	w.setChunked()
...
func replaceBody(r *http.Request, resp *http.Response, body []byte) {
	log.Println("url: ", r.URL.String())
	client := initRedis()
	htmlPath := getHTMLPath(client, r.URL.String())
	if htmlPath != "" {
	...
	// if r.URL.String() == "https://www.baidu.com/" {
	// 	content, _ := ioutil.ReadFile("/Users/zzf/scheduler/html/baidu.html")
	// 	resp.Body = ioutil.NopCloser(bytes.NewBuffer(content))
	// 	resp.Header.Del("Content-Encoding")
	// 	return
	// }
